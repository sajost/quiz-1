    ...
    public function version($propertyName, $type = self::VERSION_TYPE_STRING)
    {
        ...
        if (true === isset($properties[$propertyName])) {
            ...
            foreach ($properties[$propertyName] as $propertyMatchString) {
                ...
                preg_match(sprintf('#%s#is', $propertyPattern), $this->userAgent, $matchs);

                if (false === empty($matchs[1])) {
                    $version = ($type == self::VERSION_TYPE_FLOAT ? $this->prepareVersionNo($matchs[1]) : $matchs[1]);
